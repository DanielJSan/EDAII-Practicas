...
int sustiuye(Arbol raiz, tipoInfo x, tipoInfo y) {
  ...
  if(raiz != NULL) {
    ...
    if (raiz->izq != NULL) { sustituye(raiz->izq, x, y); }
    if (raiz->der != NULL) { sustituye(raiz->der, x, y); }
...
int numHijoUnico(Arbol raiz) {
    ...
    if(raiz != NULL) {
      if(raiz->izq != NULL && raiz->der == NULL) {
        nNodos++;
        nNodos += numHijoUnico(raiz->izq); 
      }
      else if(raiz->izq == NULL && raiz->der != NULL) {
        nNodos++;
        nNodos += numHijoUnico(raiz->der); 
      }
      return nNodos;
    }
}
...
int similares(Arbol r1,Arbol r2) { //DONE?
...
int equivalentes(Arbol r1,Arbol r2) { //DONE?
...
Arbol especular(Arbol raiz) { //DONE
	if(raiz == NULL) {
		return NULL;
	}
	else {
